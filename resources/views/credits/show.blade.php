...
@section('content-header')
	<h1>
		Pengajuan Kredit
		<small>Detail Pengajuan Kredit {{ $credit->customer->user->name }}</small>
	</h1>
@endsection
		...
		<div class="col-xs-12">
			<div class="box box-primary">
				<div class="box-header">
					<h3>Detail Pengajuan Kredit {{ $credit->customer->user->name }}</h3>
				</div>
				<div class="box-body">
					<div class="table-responsive">
						<table class="table table-hover tabler-bordered">
							<tr>
								<th>NAMA</th>
								<td>{{ $credit->customer->user->name }}</td>
								<th>EMAIL</th>
								<td>{{ $credit->customer->user->email }}</td>
                            </tr>
							<tr>
                                <th>PHONE</th>
								<td>{{ $credit->customer->user->phone }}</td>
								<th>FOTO KTP</th>
								<td><img src="{!! $credit->customer->ktp !!}" alt="{{ $credit->customer->user->name }}" width="200px"></td>
                            </tr>
							<tr>
								<th>STATUS</th>
								<td>
							...
							<tr>
								<th>HISTORY</th>
								<td colspan="3">
                                    @foreach ($credit->histories as $history)
                                        <div class="well">
                ...
                <div class="box-footer">
                    <div class="row">
                        <div class="col-md-12">
                            <hr>
                            <h4>PEMBERITAHUAN KEPADA {{ strtoupper($credit->customer->user->name) }}</h4>
                            <hr>
                            {{ Form::model($credit, ['route' => ['credits.update', $credit->id], 'method' => 'PUT']) }}
			...
			<a href="{{ route('credits.index') }}" class="btn btn-flat btn-success"><i class="fa fa-reply"></i> Kembali</a>
			{{ Form::open(['route' => ['credits.destroy', $credit->id], 'method' => 'DELETE', 'style' => 'display: inline-block;']) }}
